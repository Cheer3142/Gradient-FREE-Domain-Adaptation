...
class ClipEmbeddingModel(EmbeddingModel):
    ...
    def __init__(self):
        # self.embedding_model, self.embedding_processor  = clip.load("ViT-B/16", device="cuda")
    ...
    def embed_image(self, image_path) -> npt.NDArray:
        ...
        # ensure same device as the model
        ...
        # inputs = self.embedding_processor(image).unsqueeze(0).to("cuda")
        # image_embeddings = (
        #                 self.embedding_model.encode_image(inputs).detach().cpu().numpy()
        #             )
